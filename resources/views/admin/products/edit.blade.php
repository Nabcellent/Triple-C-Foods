            ...
            <div class="max-w-7xl sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg max-w-4xl mx-auto">
                    <div class="p-6 border-b border-gray-200">
                        ...
                        @endif
                        {{--    COMPONENT    --}}
                        <form action="{{ route('admin.kitchen.update', ['id' => $product->id]) }}" method="POST" class="w-full" enctype="multipart/form-data">
                            @csrf @method('PUT')
                            <div class="flex flex-wrap -mx-3 mb-3">
                                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                    <label for="grid-first-name">Title *</label>
                                    <input class="mb-3" name="title" type="text" placeholder="Food title" value="{{ old('title', $product->title) }}" aria-label required>
                                </div>
                                <div class="w-full md:w-1/2 px-3">
                                    <label for="grid-last-name">Price *</label>
                                    <input class="leading-tight focus:border-gray-500" name="price" value="{{ old('price', $product->price) }}" aria-label
                                ...
                                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                    <div class="bg-grey-lighter">
                                        <label class="flex flex-col items-center px-4 py-6 bg-white text-blue rounded-lg shadow-lg tracking-wide uppercase border border-blue cursor-pointer hover:bg-blue-600 hover:text-white">
                                            <svg class="w-8 h-4" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                                <path d="M16.88 9.1A4 4 0 0 1 16 17H5a5 5 0 0 1-1-9.9V7a3 3 0 0 1 4.52-2.59A4.98 4.98 0 0 1 17 8c0 .38-.04.74-.12 1.1zM11 11h3l-4-4-4 4h3v3h2v-3z" />
                                            </svg>
                                            <span class="text-base leading-normal">Upload new image</span>
                                            <input type='file' class="hidden" name="image" accept="image/*"/>
                                        </label>
                                    </div>
                                    <img src="{{ asset('images/kuku/' . $product->image) }}" class="h-16" alt="Current food image">
                                </div>
                                <div class="w-full md:w-1/2 px-3">
                                    <label for="grid-last-name">Description</label>
                                    <textarea class="h-16 text-base placeholder-gray-600 focus:shadow-outline" placeholder="short description of the food..." name="description" aria-label>{{ old('description', $product->description) }}</textarea>
                                </div>
                            </div>
